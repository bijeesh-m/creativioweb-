...
export default function CategoriesManager() {
    ...
    return (
        <div className="admin-manager">
            <div className="admin-manager-header"><h2>Categories</h2><button className="admin-btn admin-btn-primary" onClick={() => { resetForm(); setShowForm(true); }}>+ Add Category</button></div>
            {error && <div className="admin-alert admin-alert-error">{error}</div>}
            {showForm && (
                <div className="admin-modal-overlay" onClick={resetForm}><div className="admin-modal admin-modal-sm" onClick={(e) => e.stopPropagation()}>
                    <div className="admin-modal-header"><h3>{editing ? "Edit Category" : "Add Category"}</h3><button className="admin-modal-close" onClick={resetForm}>×</button></div>
            ...
            <div className="admin-table-wrapper">
                <table className="admin-table"><thead><tr><th>Name</th><th>Slug</th><th>Order</th><th>Status</th><th>Actions</th></tr></thead>
                    <tbody>{items.map((c) => (
                        <tr key={c._id}><td><strong>{c.name}</strong></td><td><code>{c.slug}</code></td><td>{c.order}</td><td><span className={`admin-badge ${c.isActive ? "badge-success" : "badge-muted"}`}>{c.isActive ? "Active" : "Inactive"}</span></td><td><div className="admin-actions"><button className="admin-btn-icon" onClick={() => handleEdit(c)}>✏️</button><button className="admin-btn-icon admin-btn-danger" onClick={() => handleDelete(c._id)}>🗑️</button></div></td></tr>
