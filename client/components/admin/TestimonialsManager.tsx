...
export default function TestimonialsManager() {
    ...
    return (
        <div className="admin-manager">
            <div className="admin-manager-header"><h2>Testimonials</h2><button className="admin-btn admin-btn-primary" onClick={() => { resetForm(); setShowForm(true); }}>+ Add Testimonial</button></div>
            {error && <div className="admin-alert admin-alert-error">{error}</div>}
            {showForm && (
                <div className="admin-modal-overlay" onClick={resetForm}><div className="admin-modal" onClick={(e) => e.stopPropagation()}>
                    <div className="admin-modal-header"><h3>{editing ? "Edit Testimonial" : "Add Testimonial"}</h3><button className="admin-modal-close" onClick={resetForm}>×</button></div>
                    <form onSubmit={handleSubmit} className="admin-form">
                        ...
                        <div className="admin-input-group"><label>Avatar</label><div className="admin-file-upload small" onClick={() => fileRef.current?.click()}>{avatarPreview ? <img src={avatarPreview} alt="Preview" className="admin-avatar-preview" /> : <div className="admin-file-placeholder"><p>Upload avatar</p></div>}<input ref={fileRef} type="file" accept="image/*" onChange={(e) => { const f = e.target.files?.[0]; if (f) { setAvatarFile(f); setAvatarPreview(URL.createObjectURL(f)); } }} hidden /></div></div>
            ...
            <div className="admin-cards-grid">
                {items.map((t) => (
                    <div key={t._id} className="admin-testimonial-card">
                        ...
                        <div className="admin-testimonial-footer"><span className="admin-stars">{"★".repeat(t.rating || 5)}</span><div className="admin-item-card-actions"><button className="admin-btn-icon" onClick={() => handleEdit(t)}>✏️</button><button className="admin-btn-icon admin-btn-danger" onClick={() => handleDelete(t._id)}>🗑️</button></div></div>
                    </div>
                ))}
