...
export default function ServicesManager() {
    ...
    return (
        <div className="admin-manager">
            <div className="admin-manager-header"><h2>Services</h2><button className="admin-btn admin-btn-primary" onClick={() => { resetForm(); setShowForm(true); }}>+ Add Service</button></div>
            {error && <div className="admin-alert admin-alert-error">{error}</div>}
            {showForm && (
                <div className="admin-modal-overlay" onClick={resetForm}><div className="admin-modal admin-modal-lg" onClick={(e) => e.stopPropagation()}>
                    <div className="admin-modal-header"><h3>{editing ? "Edit Service" : "Add Service"}</h3><button className="admin-modal-close" onClick={resetForm}>×</button></div>
            ...
            <div className="admin-table-wrapper">
                <table className="admin-table"><thead><tr><th>Title</th><th>Features</th><th>Status</th><th>Actions</th></tr></thead>
                    <tbody>{items.map((s) => (
                        <tr key={s._id}><td><strong>{s.title}</strong><br /><small>{s.shortDescription?.slice(0, 80)}...</small></td><td>{s.features?.length || 0} features</td><td><span className={`admin-badge ${s.isPublished ? "badge-success" : "badge-warning"}`}>{s.isPublished ? "Published" : "Draft"}</span></td><td><div className="admin-actions"><button className="admin-btn-icon" onClick={() => handleEdit(s)}>✏️</button><button className="admin-btn-icon admin-btn-danger" onClick={() => handleDelete(s._id)}>🗑️</button></div></td></tr>
