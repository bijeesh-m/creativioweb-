                        ...
                        <form onSubmit={handleSubmit} className="admin-form">
                            ...
                            <div className="admin-input-group">
                                ...
                                <div className="admin-file-upload" onClick={() => fileRef.current?.click()}>
                                    {logoPreview ? (
                                        <img src={logoPreview} alt="Preview" className="admin-image-preview" />
                                    ) : (
                                    ...
                                    <input ref={fileRef} type="file" accept="image/*" onChange={(e) => { const f = e.target.files?.[0]; if (f) { setLogoFile(f); setLogoPreview(URL.createObjectURL(f)); } }} hidden />
                                </div>
                            </div>
                ...
                {clients.map((c) => (
                    <div key={c._id} className="admin-item-card">
                        ...
                        <div className="admin-item-card-actions">
                            <button className="admin-btn-icon" onClick={() => handleEdit(c)}><LuPencil size={18} /></button>
                            <button className="admin-btn-icon admin-btn-danger" onClick={() => handleDelete(c._id)}><LuTrash2 size={18} /></button>
                        </div>
                    </div>
                ))}
